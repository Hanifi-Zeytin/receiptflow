...
async function analyzeWithGemini(input: { base64?: string; imageUrl?: string; headerHint?: HeaderJson }): Promise<HeaderJson | null> {
  ...
  try {
    ...
    const prompt = [
      "Türkçe fiş/fatura görselini analiz et ve SADECE aşağıdaki anahtarlarla tek bir JSON nesnesi döndür:",
      "{",
      "  isletme: string,",
      "  adres: string,",
      "  telefon: string,",
      "  tarih: string,   // dd.mm.yyyy formatında normalize et (örn. 7/5/19 -> 07.05.2019)",
      "  saat: string,    // hh:mm formatında (örn. 9:5 -> 09:05)",
      ...
      "}",
      "Kurallar:",
      "- Toplamı belirlerken 'genel toplam', 'toplam', 'toplam tutar', 'tahsilat', 'ödeme' gibi ibareleri tara; birden fazla varsa en yüksek mantıklı değeri (KDV dahil) 'genel_toplam_kdv_dahil' olarak ver.",
      "- Sayıları nokta-ayırıcı ile döndür (1234.56). '1.234,56 TL' -> 1234.56.",
      "- Değer yoksa null veya boş string kullan; uydurma değer verme.",
      "- SADECE JSON döndür, açıklama veya kod bloğu kullanma.",
